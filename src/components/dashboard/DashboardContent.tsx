...
function StatCard({ title, value, subtitle, icon, color, delay = 0 }: StatCardProps) {
  return (
    <motion.div
      ...
      className={`relative overflow-hidden rounded-2xl border bg-gradient-to-br p-5 shadow-sm transition-all hover:shadow-md ${colorMap[color]}`}
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-sm font-medium text-slate-600/80">{title}</p>
          <p className="mt-2 text-3xl font-bold text-slate-900">{value}</p>
          <p className="mt-1 text-xs text-slate-500/70">{subtitle}</p>
        </div>
...
export default function DashboardContent() {
    ...
    return (
      <div className="flex min-h-[500px] items-center justify-center p-6">
        <div className="w-full max-w-md rounded-3xl border border-red-100 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">{error || "Dashboard data was not returned."}</p>
          <button
            type="button"
  ...
  return (
    <div className="space-y-8 p-4 sm:p-6 lg:p-8">
      {/* ======================================================
          HEADER – enhanced with gradient and animation
      ====================================================== */}
      <motion.div
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
        className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-violet-600 via-indigo-600 to-blue-600 p-6 text-white shadow-lg sm:p-8"
      >
        ...
        <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-center">
          <div>
            <p className="text-sm font-medium text-violet-100">MediConnect AI Patient Dashboard</p>
            <h1 className="mt-2 text-3xl font-bold sm:text-4xl">Hello, {user.firstName} 👋</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-violet-100 sm:text-base">
              Here is an overview of your real healthcare information.
            </p>
          </div>
          <div className="flex items-center gap-3 rounded-2xl bg-white/10 p-4 backdrop-blur">
            ...
            <div>
              <p className="text-xs text-violet-100">Account status</p>
              <p className="mt-1 font-semibold">{getStatusLabel(profile.accountStatus)}</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          <a
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-violet-50 px-4 py-2 text-sm font-medium text-violet-700 transition hover:bg-violet-100"
          >
          ...
          <a
            href="/dashboard/patient/prescriptions/refill"
          ...
          <a
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-amber-50 px-4 py-2 text-sm font-medium text-amber-700 transition hover:bg-amber-100"
          >
            <FileText size={16} />
            View Records
          </a>
      ...
      <section>
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Your healthcare activity from the database.</p>
          </div>
          <button
            type="button"
            onClick={loadDashboard}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          subtitle="MediConnect AI conversations"
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        ...
        <motion.div
          ...
          className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm xl:col-span-2"
        >
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
            <div>
              <h2 className="text-lg font-bold text-slate-900">Appointment activity</h2>
              <p className="mt-1 text-sm text-slate-500">Based on your real appointment records.</p>
            </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-slate-900">Health summary</h2>
              <p className="text-xs text-slate-500">Your current patient record</p>
            </div>
          </div>
          ...
          <div className="mt-6 space-y-4">
            ...
            <div className="rounded-2xl bg-slate-50 p-4 transition hover:bg-slate-100">
              <p className="text-xs font-medium text-slate-500">Allergies</p>
              <p className="mt-1 font-semibold text-slate-900">
      ...
      <motion.section
        ...
        className="rounded-3xl border border-violet-100 bg-gradient-to-br from-violet-50 to-indigo-50/60 p-6"
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Your next scheduled consultation</p>
          </div>
        </div>
        ...
        {highlights.nextAppointment ? (
          <motion.div
            ...
            className="mt-5 rounded-2xl bg-white p-5 shadow-sm hover:shadow-md transition-all"
          >
          ...
          <div className="mt-5 rounded-2xl bg-white p-6 text-center">
            ...
            <p className="mt-3 font-semibold text-slate-700">No upcoming appointment</p>
      ...
      <section>
        <div className="mb-4 flex items-end justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Your next scheduled consultations.</p>
          </div>
          {upcomingAppointments.length > 0 && (
            <a href="/dashboard/patient/appointments" className="text-sm font-medium text-violet-600 hover:text-violet-700 flex items-center gap-1">
        ...
        {upcomingAppointments.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-slate-200 bg-white p-10 text-center">
            ...
            <p className="mt-3 font-semibold text-slate-700">No upcoming appointments</p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-slate-900">{getAppointmentDoctorName(appointment)}</p>
                      <p className="text-xs text-violet-600">{getPrimarySpecialty(appointment)}</p>
                    </div>
                  </div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Your latest prescriptions.</p>
            </div>
          ...
          <div className="mt-5 space-y-3">
            {recentPrescriptions.length === 0 ? (
              <div className="rounded-2xl bg-slate-50 p-6 text-center">
                ...
                <p className="mt-2 text-sm font-medium text-slate-500">No prescriptions yet.</p>
              </div>
            ) : (
              recentPrescriptions.map((prescription) => (
                <div key={prescription.id} className="rounded-2xl bg-slate-50 p-4 transition hover:bg-slate-100">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold text-slate-900">{prescription.prescriptionNumber}</p>
                  ...
                  <div className="mt-3 space-y-1">
                    {prescription.items.slice(0, 3).map((item) => (
                      <p key={item.id} className="text-sm text-slate-600">
                        <span className="font-medium text-slate-800">{item.medicationName}</span> — {item.dosage}
                      </p>
                    ))}
                  </div>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-slate-900">Recent laboratory results</h2>
              <p className="mt-1 text-sm text-slate-500">Your latest laboratory tests.</p>
            </div>
          ...
          <div className="mt-5 space-y-3">
            {recentLabs.length === 0 ? (
              <div className="rounded-2xl bg-slate-50 p-6 text-center">
                ...
                <p className="mt-2 text-sm font-medium text-slate-500">No laboratory results yet.</p>
              </div>
            ) : (
              recentLabs.map((result) => (
                <div key={result.id} className="rounded-2xl bg-slate-50 p-4 transition hover:bg-slate-100">
                  <div className="flex items-start justify-between gap-3">
                    <div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Your latest medical history.</p>
          </div>
        </div>

        <div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {medicalRecords.length === 0 ? (
            <div className="rounded-2xl bg-slate-50 p-6 text-center md:col-span-2 lg:col-span-3">
              <FileText className="mx-auto text-slate-300" size={30} />
              <p className="mt-2 text-sm font-medium text-slate-500">No medical records yet.</p>
            </div>
          ) : (
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Basic information from your profile.</p>
          </div>
        </div>
